...
my $timestamp;
my ($t0, $t1);
my $cwd = '/Users/jan/Desktop/MONITOR/';
my $time = localtime();
...
if ( $config{DEBUG}) {
    open FILE,"<out.txt" or die $log->error("Cannot open event file: $!");

    my $currentNodeId = '';
    my @nodeEvents;

    while (<FILE>) {
        if (/compute::([a-z0-9]+)::([a-z_0-9:]+)=([0-9.]+)/) {
            my $nodeId = $1;
            my $metric = $2;
            my $value  = $3;

            if ( $currentNodeId ne $nodeId ){
                $currentNodeId = $nodeId;
                hostHandler($nodeId, \@nodeEvents);
            }

            if ( exists $mlist{$metric} ) {
                print "$nodeId $mlist{$metric} = $value\n";
            }
        }
    }
} else {
...
sub hostHandler
{
    ...
    if ( $name =~ /[el][0-9]{4}/ ){
        ...
        foreach my $metric ( @metrics ) {
            my $metricName =  $metric->{'NAME'};
            my $metricValue =  $metric->{'VAL'};
